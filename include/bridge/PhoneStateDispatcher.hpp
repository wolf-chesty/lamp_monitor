...
#ifndef BRIDGE_PHONE_STATE_DISPATCHER_HPP
#define BRIDGE_PHONE_STATE_DISPATCHER_HPP
...
class PhoneStateDispatcher {
public:
    explicit PhoneStateDispatcher(std::shared_ptr<cpp_ami::Connection> io_conn,
                                  std::shared_ptr<DeskphoneCache> deskphone_cache);
    ~PhoneStateDispatcher();

    /// @brief Add notification message to send to deskphones to queue for dispatch.
    ///
    /// @param aors List of AoRs to dispatch \c action to.
    /// @param action PJSIP notification action to send.
    void dispatch(std::vector<std::string> aors, cpp_ami::action::PJSIPNotify action);
...
private:
    ...
    std::vector<std::pair<std::vector<std::string>, cpp_ami::action::PJSIPNotify>>
        messages_;                        ///< PJSIP notifications to dispatch.
